...
if menu == "Nova Solicitação":
    ...
    # Inicialização do formulário no session_state
    ...
    if "resetar_form" not in st.session_state:
        st.session_state.resetar_form = False

    # Reset do formulário após refresh
    if st.session_state.resetar_form:
        st.session_state["produto"] = "-- SELECIONE --"
        st.session_state["quantidade"] = 1
        st.session_state.resetar_form = False
    ...
    if st.button("Enviar Solicitação"):
        if prod == "-- SELECIONE --":
            ...
            # 🚀 Agora resetamos após o rerun (sem erro)
            st.session_state.resetar_form = True
...
if menu == "Painel Admin":
    ...
    st.write("Função disponível apenas para aline.silva")
